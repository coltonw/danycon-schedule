import Image from "next/image";
import Link from "next/link";
import Hero from "../../components/Hero";

const MedievalPage = () => {
  return (
    <>
      <Hero title="Medieval Games" subtitle="Huzzah!" />
      <section className="section">
        <div className="container">
          <h3 className="title">Medieval Historical</h3>
          <div className="grid">
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/84876/the-castles-of-burgundy">
                <Image
                  src="https://cf.geekdo-images.com/5CFwjd8zTcGYVUnkXh04hw__itemrep/img/GLJxEMRq5M4T9BZGcZ6xtDzj5OQ=/fit-in/246x300/filters:strip_icc()/pic1176894.jpg"
                  width={92}
                  height={128}
                  alt="castlesofburgundy"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/36218/dominion">
                <Image
                  src="https://cf.geekdo-images.com/j6iQpZ4XkemZP07HNCODBA__itemrep/img/_QiaiFj-LGZoqdatE-wVqNaYWx8=/fit-in/246x300/filters:strip_icc()/pic394356.jpg"
                  width={128}
                  height={128}
                  alt="dominion"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/107529/kingdom-builder">
                <Image
                  src="https://cf.geekdo-images.com/kJOP_BICVX-88sKqFRh9PQ__itemrep/img/ryZkPwqIt2kkAVb69yW9uXe6RlQ=/fit-in/246x300/filters:strip_icc()/pic1152359.jpg"
                  width={128}
                  height={128}
                  alt="kingdombuilder"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/96913/lancaster">
                <Image
                  src="https://cf.geekdo-images.com/8e4XYBOnFKGMoiW591ih8g__itemrep/img/UyCnx1xSX2JIAmgazi13Btf0ics=/fit-in/246x300/filters:strip_icc()/pic1280761.jpg"
                  width={128}
                  height={128}
                  alt="lancaster"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/129622/love-letter">
                <Image
                  src="https://cf.geekdo-images.com/T1ltXwapFUtghS9A7_tf4g__itemrep/img/SJxpAXjBIPEj9m6hHZYeTbz9wzc=/fit-in/246x300/filters:strip_icc()/pic1401448.jpg"
                  width={95}
                  height={128}
                  alt="loveletter"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/915/mystery-of-the-abbey">
                <Image
                  src="https://cf.geekdo-images.com/bu8OYIBuiFaweSwFM4usgg__small/img/b3gsW5RhyQvaQaEJLjVCg_z5cMY=/fit-in/200x150/filters:strip_icc()/pic3921982.jpg"
                  width={128}
                  height={128}
                  alt="mysteryoftheabbey"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/164928/orleans">
                <Image
                  src="https://cf.geekdo-images.com/nagl1li6kYt9elV9jbfVQw__itemrep/img/oYYWKekDiHllrWnFNef7En3gP4c=/fit-in/246x300/filters:strip_icc()/pic6228507.jpg"
                  width={91}
                  height={128}
                  alt="orleans"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/24480/the-pillars-of-the-earth">
                <Image
                  src="https://cf.geekdo-images.com/J897fuu-nl83_o90uOakVQ__itemrep/img/FBQnA2CF2RD4SITS1qJgvjVEArw=/fit-in/246x300/filters:strip_icc()/pic3691155.jpg"
                  width={128}
                  height={128}
                  alt="pillarsoftheearth"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/43528/world-without-end">
                <Image
                  src="https://cf.geekdo-images.com/r0wz1yAtjaU5e-u_hKIYeQ__itemrep/img/bp_UjaCiksPsae1fBKosAO8q7G8=/fit-in/246x300/filters:strip_icc()/pic583088.jpg"
                  width={128}
                  height={128}
                  alt="worldwithoutend"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
          </div>
          <h3 className="title">Medieval Fantastical</h3>
          <div className="grid">
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/163930/abracadawhat">
                <Image
                  src="https://cf.geekdo-images.com/ExKZ4I1R5sx-F65Gj-wDFA__itemrep/img/HqNNo6nbDxMSJWF5xfWbXQwRKkY=/fit-in/246x300/filters:strip_icc()/pic2428651.png"
                  width={128}
                  height={128}
                  alt="abracadawhat"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/238992/call-to-adventure">
                <Image
                  src="https://cf.geekdo-images.com/oz_C4Hwf1Hb6KHt6KTxJJA__itemrep/img/p5eKp_s2dmS1y9iM0xKoV0J2mNw=/fit-in/246x300/filters:strip_icc()/pic4165094.jpg"
                  width={100}
                  height={128}
                  alt="calltoadventure"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/43111/chaos-in-the-old-world">
                <Image
                  src="https://cf.geekdo-images.com/yTe9522sHL-3nhfAMH2cTg__itemrep/img/0y-xCTwtpQX2gNZkimKSC4nCBkQ=/fit-in/246x300/filters:strip_icc()/pic1318481.jpg"
                  width={128}
                  height={128}
                  alt="chaosintheoldworld"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/478/citadels">
                <Image
                  src="https://cf.geekdo-images.com/42iW4E-vOumFXRQAiSf-GA__itemrep/img/NVAwUCUVc6QWNKZNUBiIMg2nk8A=/fit-in/246x300/filters:strip_icc()/pic3239104.jpg"
                  width={128}
                  height={128}
                  alt="citadels"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/285192/destinies">
                <Image
                  src="https://cf.geekdo-images.com/oaD1ZQ3yGj6lacLdtqgdnQ__itemrep/img/hrN6o3gcBAhPaQyOhQT_n5dJ1hI=/fit-in/246x300/filters:strip_icc()/pic5558118.png"
                  width={128}
                  height={128}
                  alt="destinies"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/180325/elfenroads">
                <Image
                  src="https://cf.geekdo-images.com/09vlLOV9Brex09klGgs5gg__itemrep/img/9V7kchyyu2gPMb0Sd9oqkMqR5mY=/fit-in/246x300/filters:strip_icc()/pic2578236.jpg"
                  width={128}
                  height={97}
                  alt="elfenroads"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/293348/flesh-and-blood">
                <Image
                  src="https://cf.geekdo-images.com/3uW8k-tB-Ke0R4mIAENBSA__itemrep/img/HH8WAQY2DvmTJoLI-8Amsq9My2I=/fit-in/246x300/filters:strip_icc()/pic5871917.png"
                  width={92}
                  height={128}
                  alt="fleshandblood"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/27162/kingsburg">
                <Image
                  src="https://cf.geekdo-images.com/pHFQxD8va6vOrrNfazVzsg__itemrep/img/WzjkX3JojYT4dCGx3fwrHkgwIJc=/fit-in/246x300/filters:strip_icc()/pic253057.jpg"
                  width={128}
                  height={128}
                  alt="kingsburg"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/110327/lords-of-waterdeep">
                <Image
                  src="https://cf.geekdo-images.com/-hk8f8iGk_DyWyMrfiPBkg__itemrep/img/wo5yM8-yN8IbCzwMZsUak_m4U98=/fit-in/246x300/filters:strip_icc()/pic1116080.jpg"
                  width={101}
                  height={128}
                  alt="lordsofwaterdeep"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/96848/mage-knight-board-game">
                <Image
                  src="https://cf.geekdo-images.com/DUO2hz9AlLOH8p9ED-lCWg__itemrep/img/6OmGo3bidk3ma7U1v-oq_2pEJZI=/fit-in/246x300/filters:strip_icc()/pic1083380.jpg"
                  width={128}
                  height={92}
                  alt="mageknight"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/463/magic-the-gathering">
                <Image
                  src="https://cf.geekdo-images.com/CxJmNl4wR4InjqyNrMdBTw__itemrep/img/kBqgXHgOkEqyADW3Fk9c8KcCJRk=/fit-in/246x300/filters:strip_icc()/pic163749.jpg"
                  width={91}
                  height={128}
                  alt="magicthegathering"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/277700/merchants-cove">
                <Image
                  src="https://cf.geekdo-images.com/-JD8JLncWYMes9Q6YIgorg__itemrep/img/b1h2IBTekkN4hLIdXyH_ZxaDqXw=/fit-in/246x300/filters:strip_icc()/pic6136538.jpg"
                  width={128}
                  height={128}
                  alt="merchantscove"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/232043/queendomino">
                <Image
                  src="https://cf.geekdo-images.com/_6VtTmeI4e7cORxFWRC7Og__itemrep/img/vHPqOYE1zACpaddnHtRsGADB7pE=/fit-in/246x300/filters:strip_icc()/pic3672901.png"
                  width={128}
                  height={128}
                  alt="queendomino"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/262712/res-arcana">
                <Image
                  src="https://cf.geekdo-images.com/gcALy45JfcjyKUn6T3mBIQ__itemrep/img/kSOcgfYrimNkhTcWZzNgLa4CfvA=/fit-in/246x300/filters:strip_icc()/pic4422847.jpg"
                  width={128}
                  height={128}
                  alt="resarcana"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/278553/silver">
                <Image
                  src="https://cf.geekdo-images.com/e7ii83ya4tVD-4f1E-Alpg__itemrep/img/9Kty-BN5ys6yfww0LCvAgk35kN8=/fit-in/246x300/filters:strip_icc()/pic4741826.jpg"
                  width={128}
                  height={97}
                  alt="silver"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
            <div className="cell">
              <Link href="https://boardgamegeek.com/boardgame/120677/terra-mystica">
                <Image
                  src="https://cf.geekdo-images.com/bre12I1YiXkZr7elvriz4A__itemrep/img/pKM9lklmQLXJx2xta9eR5NSezec=/fit-in/246x300/filters:strip_icc()/pic5375624.jpg"
                  width={91}
                  height={128}
                  alt="terramystica"
                  style={{
                    maxWidth: "100%",
                    height: "auto",
                  }}
                />
              </Link>
            </div>
          </div>

          <h3 className="title">Medieval Loose Fits</h3>
          <ul>
            <li>Biblios</li>
            <li>Blood Rage</li>
            <li>Bruges</li>
            <li>
              Castles of Mad King Ludwig (medieval castles but built decidedly
              AFTER the middle ages)
            </li>
            <li>Century: Spice Road</li>
            <li>A Feast for Odin</li>
            <li>Glass Road</li>
            <li>Golem</li>
            <li>Hansa Teutonica</li>
            <li>Legend of the Five Rings</li>
            <li>Magic Maze</li>
            <li>Marco Polo I and II</li>
            <li>Medici</li>
            <li>Oh My Goods</li>
            <li>The Princes of Florence</li>
            <li>The Red Cathedral</li>
            <li>Shogun</li>
            <li>Village</li>
            <li>The White Castle</li>
          </ul>
        </div>
      </section>
    </>
  );
};

export default MedievalPage;
